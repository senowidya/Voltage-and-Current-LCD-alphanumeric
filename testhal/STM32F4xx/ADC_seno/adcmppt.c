...
static void adccallback(ADCDriver *adcp, adcsample_t *buffer, size_t n) {
  ...
  if (adcp->state == ADC_COMPLETE){
    ...
    adc_buff10=adc_buff10+samples[0+(ADC_GRP_NUM_CHANNELS*i)];
    adc_buff11=adc_buff11+samples[1+(ADC_GRP_NUM_CHANNELS*i)];
    adc_buff12=adc_buff12+samples[2+(ADC_GRP_NUM_CHANNELS*i)];
    adc_buff13=adc_buff13+samples[3+(ADC_GRP_NUM_CHANNELS*i)];
    adc_buff14=adc_buff14+samples[4+(ADC_GRP_NUM_CHANNELS*i)];
    adc_buff15=adc_buff15+samples[5+(ADC_GRP_NUM_CHANNELS*i)];
